...
const getTopic = async (from: string, token: string): Promise<ITopic[]> => {
  const data = await GET<ITopic[]>(
    `${API_ROOT}/me/recent-topic?from=${from}&size=${TOPIC_PER_REQUEST}&sf_request_type=fetch`,
    token
  );
  return data;
};
...
export async function getTopicContent(token: string) {
  ...
  const topicArr = topics.map((topic: ITopic) => {
    return async () => {
      const data = await GET<IPost[]>(
        `${API_ROOT}/Topic/${topic.id}/post?from=0&size=1&sf_request_type=fetch`,
        token
      );
      return data[0];
    };
  ...
  topicData.forEach((topic: IPost) => {
    message +=
      translateTime(topic.time) +
      "," +
      topic.title +
      "," +
      getMarkdownContent(topic.content) +
      "||";
  });
  return message;
}
